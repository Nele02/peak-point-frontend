...
export function computePeaksPerCategory(peaks: Peak[], categories: Category[]) {
	return {
		...
		datasets: [
			{
				values: categories.map((c) => peaks.filter((p) => categoryIdsOfPeak(p).includes(c._id)).length)
			}
		]
	};
}
		...
		datasets: [
			{
				values: bands.map(
					(b) =>
						peaks.filter((p) => typeof p.elevation === "number" && p.elevation >= b.min && p.elevation <= b.max)
							.length
				)
			}
		]
