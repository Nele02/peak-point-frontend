<script lang="ts">
  ...
  import {
    peaksForCategory,
    UNCATEGORIZED,
    overlayNameForCategory
  } from "$lib/services/peak-layers";
  ...
  async function updateNearbyView(p: Peak) {
    ...
    const selectedMarker = (await mapNearby.addMarker(
      p.lat,
      p.lng,
      popupNameOnly(p)
    )) as unknown as MarkerLike;
        ...
        <Card title="Peaks">
          <div class="pp-leftInner" style={`height:${OVERVIEW_HEIGHT_VH}vh;`}>
            ...
            <div class="pp-leftFooter">
              ...
              <div class="field">
                <label class="label is-small" for={radiusInputId}
                  >Nearby radius: {radiusKm} km</label
                >
                <input
                  id={radiusInputId}
