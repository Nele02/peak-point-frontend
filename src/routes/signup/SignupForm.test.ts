...
describe("SignupForm", () => {
	it("renders a normal post form", () => {
		const { container, getByRole } = render(SignupForm as any, {
	...
	it("shows message if form has one", () => {
		const { getByText } = render(SignupForm as any, {
			props: { form: { message: "Signup failed" } }
		});
	...
	it("has login redirect link", () => {
		const { container } = render(SignupForm as any, {
			props: { form: undefined }
		});
